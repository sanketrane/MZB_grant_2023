...
\geometry{left=0.55in, right=0.55in, top=0.69in, bottom=0.69in, letterpaper}  
...
\usepackage{setspace}
\singlespacing
\setstretch{0.98} % increase line spacing a little bit
...
%In humans deficiency of MZ B cells in splenectomized individuals, and their low numbers in elderly and during infancy, presents with an increased risk of multiple bacterial infections, believed to be due primarily to loss of MZ B cell derived antibodies against capsular polysaccharides~\cite{Kruetzmann_2003, Castagnola_2003, Amlot_1985}.
...
\centering
\vspace*{-4mm}
\includegraphics[width=0.3\textwidth]{Figures/bcell_matu.jpg}
\caption{\textbf{Developmental transitions within B cell subsets.}} %\textbf{(A)} Linear pathway from T1 to FO or MZ. }
\label{fig:MZ_dev}
\vspace*{-4mm}
\end{wrapfigure}
...
%Studies of B cell lineage relationships have identified CD21\hi CD23$^{+}$ T2 B cells as an exclusive MZ precursor (MZP) subset~\cite{Cariappa_2001, Srivastava_2005}. 
...
In this project, we will map B cell differentiation pathways during an ongoing immune response to study this process in detail. %develop a comprehensive understanding of MZ B cell dynamics, upon antigenic activation.}
...
Indeed, in humans MZ B cells are associated with Sjögren's syndrome~\cite{Daridon_2006, Guerrier_2012},  Graves' disease~\cite{Segundo_2001} and autoimmune vasculitis~\cite{Appelgren_2018}. % and are believed to be participating in presentation of self-antigen to auto-reactive T cells~\cite{Palm_2021} in these pathologies.
%Conversely, low MZ B cell numbers are correlated with autoimmune vasculitis in humans~\cite{Appelgren_2018}, suggestive of their protective role.
%A quantitative understanding of MZ B cell ontogeny, homeostasis and their immune response dynamics will underpin the development of therapies to regulate their numbers in autoimmune pathologies, and boost their protective activity during infancy and in old age.  
...
\begin{itemize}
    ...
    \item Population-structured probabilistic, ODE  and  PDE modeling framework to map proliferative history and maturational transitions in lymphocytes, using a powerful Rag and Ki67 double reporter mouse model. % developed specifically for this study.
    \item Integration of dynamical modeling with phylogeny-based methods to map precursor-progeny relationships and clonal trajectories in \textbf{antigen-specific B cells} using a novel antigen-inducible mouse model. % developed specifically for this study.
...
\centering
\vspace*{-4mm}
\includegraphics[width=0.4\textwidth]{Figures/age_abm.pdf}
\vspace*{-7mm}
\caption{\textbf{Schematics of the Mosaic model.} Cell-age is defined as the time since cell's exit from the BM.} % map transitions and fate-decisions in the B cell lineage.}}
\vspace*{-6mm}
...
Each age-class contains modules ($b_{0} ,\ldots, b_{3}$) carrying cells of differing B cell developmental stages -- T1, T2, FO, and MZ, respectively. %--- $b_{j}$, where $j=0,\ldots,3$ identifies .
...
\begin{tabular}{rl}
    ...
    \\
    \textbf{Aging:}  & $P$(conversion) of \khi to \klo $P_1=1 - e^{-\beta \, \tau}$  and  GFP$^+$ to GFP$^-$:  $P_2=1 - e^{-\gamma \, \tau}$.\\ %, respectively. \\
    %&Cell counts of \khi   $k_{age} = (1 - \beta) \, k_j \, z_{per}; \; \text{ and GFP}^+  g_{age} = (1 - \gamma) \,g_j  \, z_{per} $.\\
...
We will use the Markov Chain Monte Carlo (MCMC) sampling methods to estimate the joint density of model predictions to feed into the likelihood function and the model parameters. % $(\theta)$.
...
\centering
\vspace*{-4mm}
\includegraphics[width=0.22\textwidth]{Figures/ODE_approx.pdf}
\vspace*{-7mm}
...
Sum of all quadrants gives total numbers. % for the module $b_j$ in age-class $A_i$.
...
%\text{with the boundary conditions }& \quad u(t=0, g) = f(g), \quad \text{and} \quad u(t, g=1) = \phi(t).
\label{eq:gfp-pde}
\end{equation}
...
\end{equation*}
Here, ($\rho$) and ($\delta$) are \textit{per capita} rates of cell division and loss. % will be derived from the fits to the neonatal data.
...
%
%Delayed MZ B development. Efficiency in early life. Effect on clonal dynamics or clonal archiving. Neonatal versus adult MZ B repertoire. Effect of empty lymphoid niches? Imprinting?
%
%Decline in GFP high cells -- due to differences in development (\ie division or increased time in transitional stages.) or enrichment of relatively older cells in the MZ pool. Contribution from FO B pool?
%Hypothesis: Efficiency of T2 to MZ is very low. but of first t2 develop to Fo, then FO can support MZ development. hence the delay. 
%critical finding about B cell developmental pathways/dynamics. therapeutic targets to boost competence against blood-borne systemic pathogens in early life and in elderly.
%Study may also reveal processes susceptible to malignant transformations in B cell lymphoma.
%
...
%We will integrate data from a BM chimera mouse models -- generated using either a transplant-conditioning drug or a culture-based enrichment technique -- into a deterministic modeling strategy to explain MZ B cell dynamics across the lifespan.
...
Lastly, multiple reports advocate for inherent variation in B cells' sensitivity to enter and/or persist within the MZ B cell compartment based on the strength and/or frequency of BCR and Notch2 mediated interactions. %signals they receive during their lifetime~(CITES).
...
\centering
\vspace*{-4mm}
\includegraphics[width=0.41\textwidth]{Figures/BUCHI_diag.pdf}
\vspace*{-1mm}
...
\centering
\vspace*{-4mm}
\includegraphics[width=0.44\textwidth]{Figures/Ki_data.pdf}
\vspace*{-7mm}
\caption{
...
\centering
\vspace*{-9mm}
\includegraphics[width=0.21\textwidth]{Figures/Nfd_expl.png}
\vspace*{-3mm}
...
\label{eq:Nfd}
\end{equation}
%This quantity is essentially a measure of turnover; 
...
This normalization allows us to fit a single model to data from multiple mice, who may responded differently to busulfan. % treatment.
...
\centering
\vspace*{-5mm}
\includegraphics[width=0.53\textwidth]{Figures/tdt_fm_fit.pdf}
\vspace*{-7mm}
...
\centering
\vspace*{2mm}
\includegraphics[width=0.32\textwidth]{Figures/KHmodels.pdf}
\vspace*{-5mm}
...
%This ‘triangulation’ approach, fitting models simultaneously to multiple sources of data, is powerful and rarely performed in immunological settings.


%\para{\textit{Hierarchical modeling using Bayesian approach:}}
%We will divide the  data derived from Busulfan and c-kit chimera systems into two groups and fit the models described in sections~\ref{sec:buchi_models}~and~\ref{sec:buchi_models2} to them simultaneously, using the hierarchical Bayesian approach. 
%We will allow for group-level variation in model parameters, for example, net-loss rate $\lambda$ are drawn from a global distribution, which is shared across mice in these two experimental systems.
%We analyze data from Busulfan and c-Kit chimera together but as two separate groups.
%We assume the parameters in the models (such as per capita loss rate or rates of differentiation) are drawn from a global distribution which is shared across mice in UK and Munich labs. This allows us to combine the information from two (or more) experimental settings into a giant meta-analysis framework.
...
%
%\para{{2.g Minimizing  mechanistic bias in model predictions:}}
...
%An effective strategy to report uncertainty in model structure and to minimize the systematic bias in model definitions is to averaging predictions from multiple likely models~\cite{Dormann:2018}.
...
%Here, we will report the weighted average ($\widetilde{Y}$) of the predictions from the models that received strong support from the data (weights estimated using eq.~\ref{eq:model-weights}) as,
%{\small
%\vspace{-1mm}
%\begin{equation} 
...
\centering
\vspace*{-7mm}
\includegraphics[width=0.28\textwidth]{Figures/Cg1_data_mm.pdf}
\vspace*{-6mm}
...
Overlay of B220$^+$ IgM$^\text{high}$ splencoytes (blue) with CAR$^+$ cells (red). %, at day 14 post immunization.
}
\vspace*{-8mm}
...
\centering
\vspace*{-2mm}
\includegraphics[width=0.41\textwidth]{Figures/fig_7v5.pdf}
\vspace*{-5mm}
\caption{\textbf{Models of MZ B cell generation during TD immunization.}
Schematics of the models of B cell dynamics during an immune response. % leading to generation of MZ and GC B cells.
%   \textbf{(B)} Number of CAR$^+$ cells in control (red dots) and N2KO (blue dots) reporter mice, with fits from the branched, time-varying influx model  (lines).
...
Lastly, we define a `null model', in which CAR$^+$ MZ B cells are derived purely by  MZ B cell activation~(\textbf{Fig.~\ref{fig:response_model}}). % with no influx from FO and GC B cells.
In N2KO/CAR mice, differentiation of either FO or GC B cells into MZ B cells is absent, but GC B cell dynamics are similar to control/CAR mice. % \ie~Notch2 signals do not influence recruitment into or loss of B cells from GC. 
...
% for analysis of Adaptive Immune Receptor Repertoire sequencing (AIRR-seq) data~\cite{Yaari:2015}.
...
The risk of under-sampling is further mitigated by evidence that the NP-CGG specific response is highly restricted to the V186.2 heavy chain gene segment~\cite{Bothwell_1981, Bothwell_1982}; thus, we expect to generate complete trees of the top immunodominant clones that use V186.2. % this heavy chain segment.
...
%In addition, we will quantify the extent of SHM in MZ B cells to understand whether clonal heterogeneity manifests as a result of repetitive immunizations. 
