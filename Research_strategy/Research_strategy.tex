...
\includegraphics[width=0.3\textwidth]{Figures/develop_diag.png}
\vspace*{-5mm}
\caption{\textbf{Developmental transitions in splenic B cell subsets.}} %\textbf{(A)} Linear pathway from T1 to FO or MZ. }
\label{fig:MZ_dev}
\vspace*{-5mm}
\end{wrapfigure}
...
In particular, \textbf{we will infer (i) precursor-progeny relationships in the B cell lineage and (ii)  the clonal trajectories of antigen-specific cells as they diversify during immune responses, to identify processes that regulate the diversity of the MZ B cell BCR repertoire.}
...
In humans MZ B cell dysfunction is associated with several autoimmune pathologies~\cite{Palm_2021, Appelgren_2018}, such as Sj\"{o}gren's~\cite{Daridon_2006, Guerrier_2012} and  Graves' disease~\cite{Segundo_2001}.   
Outcomes of this study may underpin the development of therapies to regulate MZ B cell numbers in autoimmune disorders, or boost their numbers to augment vaccine responses. % while simultaneously providing a platform for \red{the target-discovery for vaccine design (??? meaning)}  to boost their protective activity during infancy and in old age.
...
We will also explore the effect of repetitive immunizations on the MZ B cell BCR repertoire, which has direct relevance to humans who experience much higher antigenic burdens than laboratory mice.
...
\centering
\vspace*{-6mm}
\includegraphics[width=0.38\textwidth]{Figures/Ont_data4.pdf}
\vspace*{-2mm}
...
Ki67 is a nuclear protein which is expressed upon entry into the cell cycle and remains detectable for $\sim$4 days on lymphocytes~\cite{Gossel_2017, Verheijen_2020}. It is therefore suited for measuring the typically slow rates of self-renewal at steady state in adults, but it is a relatively blunt instrument for resolving the more rapid proliferation (inter-division times < 4 days) seen in neonates. 
...
\textbf{1.c The Mosaic model: a probabilistic approach to quantifying B cell development:}
...
\centering
%\vspace*{1mm}
\includegraphics[width=0.38\textwidth]{Figures/age_abm5.pdf}
\vspace*{-7mm}
\caption{
\textbf{(A)} Schematic of the mosaic model. %Structure of age-classes in a mouse of age $t$. A cell's age is defined as the time since its ancestor left the bone marrow (BM).
...
We divide this age distribution into $N$ age-classes ($A_{0} ,\ldots, A_{N-1}$)  of width $\tau$ (\textbf{Fig.~\ref{fig:age_ABM_sketch}A}), such that the youngest age of cells in $A_{i}$ is $i \, \tau$. %, \blue{ {\ie} $N = t/\tau$ and varies with the age of the mouse.}
The total number of cell age classes in a mouse of age $t$ is $N=t/\tau$.
Each age class is associated with modules ($b_{0} ,\ldots, b_{3}$) representing B cells at four stages of development -- T1, T2, FO, and MZ. Each module is a vector of cell numbers and Ki67 and GFP expression levels. %--- $b_{j}$, where $j=0,\ldots,3$ identifies .
We assume unidirectional flows between these developmental stages, and define $\psi_{j, I}$ and $\psi_{j, E}$ to be rates of  influx and efflux for the module $b_j$
%considering all possible pathways as described in
\textbf{Fig.~\ref{fig:age_ABM_sketch}B}.
For example, for the $b_1$ module (T2 ), $\psi_{j,E} = \psi_1 + \psi_2$.
The model allows rates of cell division ($\rho_{j}(n)$) and loss ($\delta_{j}(n)$) to vary with cell age and/or between modules. 
We define a cell's age as the time since its ancestor left the bone marrow (BM) and assume it to be heritable following division or differentiation.

\textbf{\textit{Operation:}}
We will initialize this model on day 1 after birth with $1/{\tau}$ age-classes uniformly populated by GFP$^{+}$ and \khi cells. % ({\ie}~$ N_0= ).
%We assume all cells at $t_{0}$ are GFP$^{+}$ and {\khi}.
With time steps of size $\tau$, cells move from  age class $A_n$ to $A_{n+1}$.
%Therefore, at each step the total number of age-classes increases by~1~(\textbf{Fig.~\ref{fig:age_ABM_sketch}A}).
...
%%\vspace{1mm}
%\begin{mybox}
%\textbf{\textit{Dynamics of the module $b_j$ in age-class $A_n$:}} at time $t$, all cells have age $a= n\, t$. \\
%
%\vspace{-3mm}
%
%The numbers of (1) \klo GFP$^-$, (2)  \khi GFP$^-$, (3) \khi GFP$^+$, and \klo GFP$^+$  are: $[z_j^1, z_j^2, z_j^3, z_j^4]$. \\
%%We will employ a sequential event-mapping algorithm to update the state-vector of module b$_j$\textbf{:}\\
%At time $t+\tau$, $a=n\,(t+\tau)$ and $b_j$ is updated to $b_j^*$  by mapping fate-decision events sequentially:\\
%\textbf{(F1) death} $\rightarrow$ 
%\textbf{(F2) efflux} $\rightarrow$ 
%\textbf{(F3) aging} $\rightarrow$ 
%\textbf{(F4) division} $\rightarrow$ 
%\textbf{(F5) influx}.
%For $s = 1\ldots4$,
%
%\begin{tabular}{ll}
%\\
%    \textbf{(F1)} &Number of surviving cells =  $y_j^s \sim \text{Binom}(z_j^s, 1-P_\text{death})$, where $P_\text{death}= 1 - e^{-\delta_{j}(a) \, \tau}$. \\ %, for $s = 1\ldots4.$ \\
%    %\\
%    \textbf{(F2)} &Number of cells that persist = $x_j^s \sim \text{Binom}(y_j^s, 1- P_\text{eff})$, where $P_\text{eff}= 1 - e^{-\psi_{j, E} \, \tau}$.  \\
%    %& Number of persisting \khi cells: $k_p = \kappa_j \cdot z_p \quad \text{ and GFP}^+ \text{cells}: f_p = \phi_j \cdot z_p$.\\
%    & Efflux is independent of cells' Ki67 or GFP status, such that $ z_{j+1}^s = y_j^s - x_j^s$. \\  %, for $s = 1\ldots4.$\\
%    %\\
%    \textbf{(F3)}  & We define $\beta$ and $\gamma$ as the rates of conversion of \khi to \klo and GFP$^+$ to GFP$^-$. \\ %, respectively.
%    %& $P_1 = \beta \, \tau$  and  \prob(GFP$^+ \rightarrow$ GFP$^-$):  $P_2 = \gamma \, \tau$. \\     
%    & \khi fraction of persisting cells:  $k_{p} \sim \text{Binom}(k_j  \cdot z_{p},  1-P_{1}), \text{ where }P_1 = \beta \, \tau.$ \\
%    & GFP$^+$ fraction of persisting cells: $g_{p} \sim \text{Binom}(g_j  \cdot z_{p},  1-P_{2}),  \text{ where }P_2 = \gamma \, \tau.$ \\
%    %\\
%    \textbf{(F4)}  &$P_\text{div}= 1 - e^{-\rho_{j}(a) \, \tau} \; \implies$
...
%    &Number of GFP$^+$ cells after division events: $f_{d} \sim \text{Binom}(f,  P_3)$.\\ 
%    %\\
%    \textbf{(F5)}  &Number of new cells entering from the precursors $z_{\nu} =  \psi_{j, I} \; z_{j-1} \; \tau,  \;\; j\neq0$ \\
%\end{tabular}
%
%\vspace{1mm}
...
\vspace{-3mm}
At time $t$, cell numbers and \khi and GFP$^+$ fractions are  $b_j = [z_j, y_j, x_j]; \; y_j, x_j \in [0, 1]. $ \\ %
At time $t+\tau$, we update $b_j$ to $b_j^*$  by modeling events sequentially:\\
...
\\
    ...
    %\textbf{(S2):} &Number of cells that migrate to next compartment = $z^{\prime}_{j+1} \sim \text{Binom}(z_s, P_\text{eff})$, where $P_\text{eff}= 1 - e^{-\psi_{E, j} \, \tau}$. \\
    & Differentiation is independent of cells' Ki67 or GFP status, such that the number and \\
    & \khi and GFP$^+$ fractions of cells entering $j+1$ are $z^{\prime}_{j+1} = z_s - z_p, \; y^{\prime}_{j+1} = y_j, \text{ and } x^{\prime}_{j+1} = x_j.$  \\
    %
    %& $y^{\prime}_{j+1} = \lfloor (y_j/z_j) \, z^{\prime}_{j+1} \rfloor \text{ and } x^{\prime}_{j+1} = \lfloor (x_j/z_j) \, z^{\prime}_{j+1} \rfloor.$  \\
    %& Numbers of total, \khi and GFP$^+$ cells that persist in $b_j$: $z_p = z_s - z^{\prime}_{j+1}, y_p = y_j - z_s, x_p = x_j - z_s, $.\\
    \\
    \textbf{(S3):}  & We define $\beta$ and $\gamma$  to be the rates of stochastic conversion of \khi to \klo and GFP$^+$ to GFP$^-$. \\ %, respectively.
    ...
    \\
    \textbf{(S4):}  & Total number of cells that divide is $z_{d} \sim \text{Binom}(z_{p} , P_\text{div}), \text{ where } P_\text{div}= 1 - e^{-\rho_{j}(n) \, \tau}.$ \\
    & Number of new \khi cells: $k_d = 2 \cdot z_{d}$. \\
    & Number of GFP$^+$ cells that undergo division: $g \sim \text{Binom}(g_{p}, P_\text{div})$. \\
    & We define $\epsilon$ to be the rate of conversion of GFP$^+$ to GFP$^-$ due to division.\\
    & Number of GFP$^+$ cells after division events: $g_{d} \sim \text{Binom}(g,  P_3), \text{ where } P_3 = 1 - e^{-\epsilon \, \tau}$.\\ 
    \\
    \textbf{(S5):}  & Total numbers, and numbers of \khi and GFP$^+$ cells entering state $j$ from precursor $j-1$ \\
    & $z_{\nu} =  z^\prime_{j-1}, \; k_\nu = y^\prime_{j-1} \, z_\nu, \; \text{ and } g_\nu = x^\prime_{j-1}\, z_\nu;  \;\; j\neq0.$ \\ %\psi_{j, I} \; z_{j-1} \; \tau,  \;\; j\neq0$ \\
\end{tabular}
...
%\textbf{\textit{Nested modeling strategy:}} 
We will consider model instances representing candidate pathways of MZ B cell development; for example, differentiation of (i) T1, T2, and FO ($\psi_{3}\neq0, \psi_{4}\neq0$) or (ii) only T2 ($\psi_{3} = \psi_{4} = 0$) into MZ B cells~(\textbf{Fig.~\ref{fig:age_ABM_sketch}B}).
We will also explore models in which division and loss rates vary with cell age and/or density, with various functional forms (exponential, sigmoid), motivated by studies that suggest both effects influence lymphocyte dynamics~\cite{Rane_2022, Rane_2018, Hogan_2015,  De_Boer_2013, DeBoer_2001, Min_2003, Freitas_1995}.
In the latter setting, our mosaic model is more tractable than a PDE model, whose solutions tend to become numerically unstable when  density dependent processes are involved.
Crucially, since the mosaic model aggregates lymphocyte populations into age and phenotypic classes, \textbf{it requires significantly less computational resources than typical stochastic simulation algorithms}, such as agent-based models or  Doob-Gillespie solvers, which track individual cells or reactions. % and therefore have heavy computational load. % instead of cohorts of cells binned as age-classes. 
...
We will use  Markov Chain Monte Carlo methods to estimate the model parameters and the joint density of model predictions. % that feeds into the likelihood function. % $(\theta)$.
...
%The joint density of  in each animal is defined as, .
...
%The posterior distribution of model parameters will be sampled from the joint prior density, using the Metropolis-Hastings algorithm. % of MCMC.
We will use the Metropolis-Hastings algorithm with broad priors to generate samples of model parameters from the joint posterior density.
%We will estimate the probability of each sampled set by averaging likelihoods from $\sim1000$ simulations to generate the posterior distribution. 
The likelihood is computed by averaging predictions across many simulations.
Models and the sampler will be coded in C++ for efficiency. % for efficient simulations and sampling.
...
Essentially, we will divide the age-class in four quadrants based on GFP and Ki67 status (+ve or -ve). % within each module in an age-class. 
...
Here, the sum of all quadrants gives total numbers. % for the module $b_j$ in age-class $A_i$.
The power of this approach is that ODE models can be solved more quickly than probabilistic models and fitted using established algorithms in \textit{R} or \textit{Stan}.

\textbf{\textit{Modeling continuous representations of cell states:}}
The data will provide continuous measurements of GFP and Ki67 intensity, which we reduce to binary variables in the Mosaic and ODE models. %do not track continuous changes in GFP expression and therefore 
To make full use of this information,  we will explore PDE models that track continuous changes in cells' internal states.

\textbf{\textit{Population structured PDE model:}}
We assume that GFP expression ($g$) declines with cell age exponentially at rate $\beta$ and is halved following cell division.
We will empirically construct the initial distribution of GFP expressing cells ($f(g)$) at time $t_{0}$ from the data and
assume that new cells entering the compartment have the highest GFP intensity ($g=1$).
%The total influx of new cells is proportional to the total size of the precursor pool and will be defined as $\phi(t)$.
The total influx into the MZ pool for $t>t_0$ is given by $\phi(t)$.
The PDE that tracks the population density $u(t, g)$  of GFP expressing cells at time $t$ is then
\begin{equation}
\begin{aligned}
\small
\frac{\partial u}{\partial t}  - \beta \,g \, \frac{\partial u}{\partial g} = 2\,  \rho(t) & \, u(t, 2g) - \rho(t) \, u(t, g) - \delta(t)  \, u(t, g); 
%\qquad \overbrace{ u(t=0, g) = f(g); \; u(t, g=1) = \phi(t)}^{\text{boundary conditions}}.
\\
\text{Boundary conditions: }& \;  u(t=0, g) = f(g), \;\; u(t, g=1) = \phi(t), \;\; u(t,g>1) = 0.
\label{eq:gfp-pde}
\end{aligned}
\end{equation}
%Here, $g$ is the GFP expression normalized to maximum GFP intensity in MZ B cells, such that $0 \le g \le 1$ and 
Here $\rho$ and $\delta$ are \textit{per capita} rates of cell division and loss. This PDE can be  solved numerically to derive the GFP expression profile of the population at time~$t$, which can be compared to the observed profile by rescaling it and binning into finite intervals. % from threshold GFP intensity ($\bar{g}$) to 1, to obtain the fraction of GFP$^{+}$ cells.
Eq.~\ref{eq:gfp-pde} can be extended to track simultaneously the expression of GFP ($g$) and Ki67 ($k$). We took a similar approach modeling of age and Ki67  among naive T cells~\cite{Rane_2022}. 
...
We will validate these models and their predictions using mouse strains that carry B cell-specific Notch2$^{+/-}$ and Notch2$^{-/-}$ mutations, which contain a dose-dependent reduction in MZ B cell numbers~\cite{Witt_2003, Saito_2003}.
...
This out-of-sample prediction approach reduces our reliance on information criteria for performing model selection. % and allows us to compare developmental efficiency of MZ B cells from T2 and FO B cells.
Further, it provides the means \textbf{to predict clinical outcomes in MZ B cell deficient conditions, such as vaccine responses in infants and the elderly, where MZ B cell numbers are substantially lower than in healthy adults.
}
...
Identifying the precise nature of heterogeneity in dynamics within cell populations is challenging. For example, DNA labeling experiments equivocally support diverse models of turnover and population structure within in multiple  T and B cell lineages~\cite{Asquith_2002, Westera_2013, De_Boer_2013, Bonhoeffer_2000, Ganusov_2010, Ganusov_2013, Schittler_2013}.
...
For example, a study showing long-term maintenance of MZ B cells in the absence of influx from BM-derived precursors~\cite{Carvalho_2001, Hao_2001} suggest a compensatory survival or self-renewal mechanism (quorum sensing).   
There is also an \red{age-associated decline in in the BCR diversity of MZ B cells~\cite{Birjandi_2011, Cortegano_2017}.}
Lastly, multiple reports advocate for variation in B cells' sensitivity to enter and/or persist within the MZ B cell compartment, based on the strength and/or frequency of BCR~\cite{Cariappa_2001, Seo_2001, Samardzic_2002,  Pillai_2005, Martin_2000, Chen_1997} and Notch2~\cite{Tanigaki_2002, Saito_2003, Witt_2003, Hozumi_2004, Hammad_2017, Hampel_2011, Lechner_2021} mediated interactions. %signals they receive during their lifetime~(CITES).
\textbf{We hypothesize that MZ B cells are regulated by an interplay of multiple processes and aim to identify cell-intrinsic and -extrinsic factors governing their numbers and BCR clonal diversity.
}

\begin{wrapfigure}{r}{0.36\textwidth}
\centering
\vspace*{-6mm}
\includegraphics[width=0.36\textwidth]{Figures/BUCHI_diag.pdf}
\vspace*{-7mm}
\caption{\textbf{MZ B cell dynamics in busulfan chimeras.}
\textbf{(A)} Strategy to generate busulfan chimeras.
\textbf{(B)} Flow-cytometry plots (at $\sim$5 weeks post transplant) showing strategy to identify donor and host cells in MZ (CD23$^-$, IgM$^\text{hi}$, CD21$^\text{hi}$) subset in B cells from recipient mice.% FSC is proxy for cell-size. t
}
\label{fig:BUCHI}
\vspace*{-5mm}
\end{wrapfigure}
...
Our studies providing unifying explanations of dynamics of multiple lymphocyte subsets across the lifespan using similar fate-mapping approaches %, and an innovative carbon-dating ,
serve as proofs-of-principle for this strategy~\cite{Rane_2018, Verheijen_2020, Rane_2022, Mold_2019}.
...
The power of this approach is that it allows us to study the unperturbed behavior of MZ B cells at steady-state, without complications from, for example, the increased rates of cell division usually observed under lymphopenic conditions.
%\textbf{Strategy:} 
We will generate BM chimeras (> 60 recipients of ages between 8 -- 20 weeks) using an optimized dose of busulfan~(10~$\mu$g/g of body weight~\cite{Hogan_2019}) and will track the donor proportions within MZ B cell~(\textbf{Fig.~\ref{fig:BUCHI}B}) and their precursors, over a 1-year window.

\begin{wrapfigure}{l}{0.45\textwidth}
\centering
\vspace*{-3mm}
\includegraphics[width=0.45\textwidth]{Figures/Ki_data.pdf}
\vspace*{-7mm}
\caption{
...
Our pilot study revealed that total (donor + host) MZ B cell numbers decline only slowly throughout life (\textbf{Fig.~\ref{fig:BUCHI_data}A}, left panel), consistent with previous observations in WT mice~\cite{Hao_2001, Carvalho_2001}.
The frequencies of donor-derived cells within BM precursor and transitional B cell populations in the spleen all attain steady state as early as 2 weeks post BM transplant (BMT), in line with our previously published data~\cite{Verheijen_2020}. 
...
We will also use concurrent measurements of Ki67 to determine rates of proliferation among MZ B cells and their precursors.
Analysis of Ki67 expression in our pilot stufy revealed a unimodal shift in its distribution through development of B cells in BM and spleen (Pre-B, transitional, and mature B cells; \textbf{Fig.~\ref{fig:BUCHI_data}B}), indicating a progressive decline in proliferative activity throughout development.
...
This dynamic can derive from residual Ki67 expression on newly entered cells, and/or from  differences in the dynamics  of newly generated (donor) MZ B cells and their more established (host) counterparts. We will distinguish between these possibilities in our modeling framework.

\para{\textit{Inter-individual scaling of the donor fraction:}}
The levels at which the donor fractions stabilize in each subset vary across mice, due to differences in the degree of depletion and replacement of host HSC by busulfan treatment and BMT. We accommodate this variation by normalizing the donor fraction (chimerism) in any given B cell subset to the donor fraction in their (putative) precursor. We refer to this as the normalized chimerism $f_\text{d}$;
\begin{wrapfigure}{r}{0.25\textwidth}
\centering
\vspace*{-4mm}
\includegraphics[width=0.25\textwidth]{Figures/Nfd_expl.png}
\vspace*{-5mm}
...
\begin{equation}
\small
f_\text{d} = \frac{\text{Donor cell numbers}}{\text{Total cell numbers} \times \chi}, \;\; \text{where } \chi  =\text{chimerism of precursor}
\label{eq:Nfd}
\end{equation}
...
The kinetics of infiltration of donor cells can reveal the  substructure of lymphocyte populations.
...
Further, the kinetic with which $f_\text{d}$ stabilizes can be used to infer the net loss rate -- the aggregate of death, differentiation and proliferative renewal -- and to detect heterogeneity in this loss rate across the population.

\para{{2.c Modeling  MZ B cell dynamics in busulfan chimeras:}} \label{sec:buchi_models}
A simple description of MZ B cell homeostasis is a constant birth-loss (neutral) model, in which new cells enter at a constant rate ($\phi$) and subsequently exhibit constant rates of division ($\rho$) and loss ($\delta$).
In this model, $1/\rho$ and $1/\delta$ are the expected inter-division  and residence times, respectively.
...
The dynamics of {\khi} ($H$) and {\klo} ($L$) MZ B cells are then 
...
We assume that eqns. (\ref{eq:neutral_ki}) hold identically for donor and host cells, and can be combined to yield the dynamics of donor ($M_{d}$), host ($M_{h}$) and total  ($M = M_{d} + M_{h}$) MZ B cell numbers:
{\small
\begin{equation}\small
...
Here $\lambda$ is the net loss rate ($\delta - \rho$) , the balance of loss compensated by self-renewal, and $\chi$ is the chimerism of the precursor population.
Using eqs.~\ref{eq:Nfd} and \ref{eq:neutral_chi}, the normalized chimerism $f_\text{d}$ among  MZ B cells is  given by
{\small
\begin{align} 
\frac{d}{dt} f_\text{d}(t) = \frac{d}{dt} \bigg(\frac{M_{d}(t)}{\chi \, M(t)} & \bigg)  = \frac{\phi \, P(t)}{M(t)} \, \big(1-f_\text{d}(t)\big); \quad \text{for }\chi, M(t) > 0. \label{eq:nfd_derive1} \\  
\text{At steady state, } \frac{d}{dt} M(t) = 0, \; & \text{ and so }  \frac{\phi \, P(t)}{M(t)} = \lambda  \; \rightarrow  \frac{d}{dt} f_\text{d}(t) = \lambda \, \big(1 - f_\text{d}(t)\big). 
\label{eq:nfd_derive2}
\end{align}
}%
With total cell numbers at steady state, $f_\text{d} \rightarrow 1$ at a rate determined solely by $\lambda$ (eq.~\ref{eq:nfd_derive2}).  Intuitively, $\lambda$ reflects the persistence of MZ B cell clones within the population; $\ln(2)/\lambda$ is the clonal half life. 
For a population out of equilibrium, the kinetic of $f_\text{d}(t)$ is governed by both $\lambda$ and  the rate of influx from the precursor, $\phi(t)P(t)$~(eq.~\ref{eq:nfd_derive1}).
...
Explaining the dynamics of MZ B cells may require extensions of the neutral model in which the \textit{per capita} rate of influx from the precursor population~$\phi$, the rate of division $\rho$, or the rate of loss $\delta$  vary with host age (time-dependence), perhaps due to changes in  the cellular micro-environment.
We will define three sub-models that allow time-dependence in one process at a time;  within each sub-model we will test different forms (exponential, sigmoid, \textit{etc.}) of variation with mouse age.
...
We will define extensions of theneutral model in which MZ B cell turnover, division, and/or the rate of recruitment of new cells vary with the population density through a quorum-sensing mechanism such as competition for a shared limiting resource.
This strategy will allow us to understand whether MZ B cell numbers % (0-1\% of total B cells in Notch2$^{-/-}$, 2-4\% in Notch2$^{+/-}$, and 6-8\% in WT controls)
exert feedback on the influx of new cells or on their division or retention within the niche.
...
\vspace*{-7mm}
\caption{{Models of kinetic heterogeneity among MZ B cells.}}% and clonal diversity.}}
\label{fig:khmodels}
\vspace*{-6mm}
\end{wrapfigure}
...
In both of these models, an initial rapid increase in  chimerism, due to replacement of cells with faster turnover, is followed by a more gradual approach to stable chimerism as the slower subpopulation is replaced.
...
We will define fast and slow subsets within both donor and host compartments as in eq.~\ref{eq:neutral_ki}, which then can be combined to derive the proportions of \khi  cells within donor and host populations.
The kinetics of the pool size and normalized chimerism will be determined as in the neutral model, using eqs.~(\ref{eq:neutral_chi}-\ref{eq:nfd_derive2}).
...
We assume that the homeostatic fitness of each cell entering the MZ B compartment follows a lognormal or power law distribution ($q_{\phi}(\lambda)$) and its ability to persist within the population is inherited by the progeny after self-renewal. 
The dynamics of donor ($M_{d}$), host ($M_{h}$) and total ($M$) MZ B cells can be defined as in eq.~\ref{eq:neutral_chi},
\begin{equation} \small
%\text{}, \quad
\dot M_{d}(\lambda, t) = \phi \, P(t) \chi \, q_{\phi}(\lambda) - \lambda \, M_{d}(\lambda, t) , \quad 
\dot M_{h}(\lambda, t) = \phi \, P(t) (1-\chi) \, q_{\phi}(\lambda) - \lambda \, M_{h}(\lambda, t),  \quad 
\dot{M}(\lambda, t) = \phi \,P(t) \, q_{\phi}(\lambda) - \lambda \, M(\lambda, t).
\end{equation}
In this scenario, MZ B cell clones with lower net loss rate $\lambda$ (higher fitness)  will accumulate over time.
This model predicts faster replacement of host MZ B cells in younger chimeric animals than in older recipients, as fitter clones are enriched in the MZ B pool with age.
...
The time courses of total counts, donor chimerism, and \khi fraction of MZ B cell precursors will be described empirically using phenomenological functions and will be validated separately using observations from busulfan chimeric mice.
...
\centering
\vspace*{-5mm}
\includegraphics[width=0.45\textwidth]{Figures/tdt_fm_fit.pdf}
\vspace*{-3mm}
...
We compared the fitted neutral and time-dependent models of MZ B cell replacement in busulfan chimeras.
In this preliminary analysis, the time-varying-loss model in which the MZ B cell loss rate ($\delta$) increases exponentially with time received strongest ($W$~=~98\%) statistical support from the data~(\textbf{Fig.~\ref{fig:model-fit}})  over the neutral model and the models with time-dependence in influx or division rates.
The time-varying-loss model favored differentiation from FO B cells as the dominant pathway of MZ B cell generation at steady-state, with $\sim$8\% of MZ B cells are replaced daily.
The model yields an expected lifespan of MZ B cells of $\sim$20 days in a 6 weeks old mouse, which halves almost every year.
Also, MZ B cells divide very rarely  (mean interdivisional time is $\sim$3 months), suggesting that their Ki67 expression is largely inherited from their precursors,  likely highly proliferative B cell progenitors in the BM. 
...
We will use  polyvinyl alcohol based media~\cite{Wilkinson_2020} to selectively expand HSCs from magnetically purified c-Kit$^+$ BM cells in cultures.
...
VH81x pairs with endogenous light chains to generate a limited BCR repertoire, capable of recognizing self-antigens, and has been shown to bias development of B cells towards the MZ B cell fate~\cite{Martin_2000, Hammad_2017}.
%B cells with VH81x transgene can recognize self-antigens and show a bias to develop into MZ B cells~\cite{Martin_2000, Hammad_2017}.
We will compare the kinetics of donor cell accumulation in chimeras generated using VH81x and wild-type donors.
This comparison will allow us to study the effect of BCR signaling on MZ B cell development and maintenance and will provide a method of external validation for the models that assume heterogeneity in MZ B cells, {\ie} the birth-death privilege and kinetic heterogeneity models. % versus models with heterogenous population structure
If necessary, we will also use transgenic donors carrying the MD2 heavy chain that drives development away from the MZ fate~\cite{Martin_2000}.
...
To achieve this, we will follow the dynamics of CD45.1 MZ B cells adoptively transferred  into congenic (CD45.2)  Notch2 mutant mice. % carrying CD19 driven (B cell specific) Notch2$^{+/-}$ and Notch2$^{-/-}$ mutations, which contain a dose-dependent reduction in MZ B cell numbers~\cite{Witt_2003, Saito_2003}.
...
Conceptually, the kinetic heterogeneity model accommodates a possibility in which transferred cell numbers decline initially, as the fast population depletes, but then are maintained stably by a longer-lived, self-renewing slow population. 
...
\red{This method seems suitable for estimating parameters given by mutiple models, or for making predictions about the next observation in a time series - can you connect it to what you're trying to do (which is largely to select models)? Perhaps this isn't needed?}


%\vspace*{-2mm}
\subsection*{Aim 3: Model MZ B cell fate-determination during immune responses}
\vspace*{-1mm}
In this aim, we will use dynamical models to map the fate(s) of mature FO, MZ and germinal center (GC) B cells responding to a T-dependent (TD) antigen, during a primary immune response.
...
\textbf{We postulate that antigenic activation can trigger responding FO and/or GC B cells to develop into MZ B cells, potentially depending on the availability of Notch2 signals. }
Here, we will quantify the efficiency and the kinetic of differentiation in the B cell lineage during an ongoing immune response, using a \red{novel} fate-reporter mouse model that specifically tracks activated B cells. \red{only novel if it's not been used in published work...}


\para{{3.b Insights from preliminary data -- interplay of BCR and Notch2 signals during B cell responses:}}
We \red{careful with we!. Safer to say `it was recently discovered'?} recently discovered that B cell-specific constitutive activation of Notch2 signaling diverts differentiation of antigen-activated B cells away from GC and towards an MZ B cell fate~\cite{Babushku:2022}.
Additionally, we report $\sim$100-fold up-regulation of Notch2 expression upon \textit{in vitro} stimulation of purified FO B cells using BCR and CD40 agonists (for 12-14h)~\cite{Babushku:2022} and elevated numbers of MZ B cells in mice carrying B cell-specific constitutively active CD40 signaling~\cite{Homig_Holzel_2008}.
...
In this mouse strain, B cells responding during an immune response will permanently express CAR reporter protein and can be tracked over time by antibody staining and flow cytometry~\cite{Heger_2015}.
These mice have normal ligand-dependent Notch2 signaling.

\textbf{(2) N2KO/CAR mice:} 
In these mice, B cells activation triggers inactivation of Notch2 signaling and permanent expression of the CAR reporter~\cite{Besseyrias_2007}.
...
In both models, we included a contribution from direct activation of MZ B cells to the CAR$^+$ MZ B cell pool. %, defined by a \textit{per capita} rate of flux $\beta$.
Lastly, we define a `null model', in which CAR$^+$ MZ B cells are derived only from the activation of CAR$^-$ MZ B cells~(\textbf{Fig.~\ref{fig:response_model}A}). % with no influx from FO and GC B cells.
We assume that in N2KO/CAR mice, differentiation of either FO or GC B cells into MZ B cells is absent, but GC B cell dynamics are similar to those in control/CAR mice. % \ie~Notch2 signals do not influence recruitment into or loss of B cells from GC. 
We performed a series of immunization experiments using these reporter mice (n > 80) and measured the CAR$^+$ fractions and total numbers of FO, GC and MZ B cells at 8 timepoints up to a month.
CAR$^+$ MZ B cells appear as early as day 4 post NP-CGG challenge, and their numbers continue to increase up to a month post immunization.
To describe this kinetic, we will explore the following substructures in branched, linear and null models; %  during the TD response.
...
We will define empirical functions describing the                                                                                                                                                                                              time courses of  precursor numbers -- CAR$^+$ FO and CAR$^-$ MZ B cells --  to model the influx into MZ and GC compartments.
Models will be encoded as ODE systems in  \textit{Stan}.
We will estimate  parameters and rank models using the LOO method (section 1.d).
As groundwork for this proposal, we took this approach to compareneutral and time-dependent models of CAR$^+$ MZ B cell generation.
We found that the branched model with time-varying rate of influx of FO B cells in MZ and GC compartments received strongest support  (fits shown in~\textbf{Fig.~\ref{fig:response_model}B}).  
...
As the process of affinity maturation, where B cells undergo rapid mutation and selection, happens concurrently with these differentiation events, antigen-specific MZ B cells  may accrue differential degrees of somatic hypermutation (SHM) in their BCR repertoire~(\textbf{Fig.~\ref{fig:clone-tree}A}).
...
using a phylogenetic computational approach to analyze BCR sequencing data, with the goal of inferring the trajectories of antigen-specific B cells as they emerge in different subsets. % and quantify  and to \red{quantify} their spread among different subsets.

\begin{wrapfigure}{r}{0.26\textwidth}
\centering
\vspace*{-4mm}
\includegraphics[width=0.26\textwidth]{Figures/clone-tree.pdf}
\vspace*{-7mm}
...
We will analyze sequencing data using {Seurat}~\cite{Hao:2021} (RNA-seq) and the Immcantation (BCR-seq, Kleinstein lab) packages. % for the analysis of Adaptive Immune Receptor Repertoire sequencing data~\cite{Yaari:2015}.
...
The density of phylogenetic trees depends on the sampling depth.
Missing branches and tips in  clonal trees due to under-sampling of  members of clonal families may obscure the characterization of clonal lineages.
...
In humans, the MZ B cell BCR repertoire is a  mixture of un-mutated and `memory like' somatically mutated clones, while in mice most MZ B cells express un-mutated BCRs~\cite{Kibler_2021, Weller_2004, Cerutti_2013, Pillai_2005}.
...
Sequencing experiments will use healthy animals of the same age and sex.
%
%\vspace{1mm}
