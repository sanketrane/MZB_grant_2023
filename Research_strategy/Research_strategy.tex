...
\centering
\vspace*{1mm}
\includegraphics[width=0.36\textwidth]{Figures/age_abm5.pdf}
\vspace*{-6mm}
\caption{
\textbf{(A)}~Schematics of the mosaic model. %Structure of age-classes in a mouse of age $t$. A cell's age is defined as the time since its ancestor left the bone marrow (BM).
...
We divide this age-distribution into $N$ age-classes ($A_{0} ,\ldots, A_{N-1}$)  of width $\tau$ (\textbf{Fig.~\ref{fig:age_ABM_sketch}A}), such that age of cells in $A_{n}$ is $n \, \tau$. %, \blue{ {\ie} $N = t/\tau$ and varies with the age of the mouse.}
...
We will initialize this model on day 1 after birth with $1/{\tau}$ number of age-classes, which are uniformly populated by GFP$^{+}$ and \khi cells. % ({\ie}~$ N_0= ).
%We assume all cells at $t_{0}$ are GFP$^{+}$ and {\khi}.
The model then takes time steps of size $\tau$ and at each step we move cells from age-class $A_n$ to $A_{n+1}$. % are incremented by 1 {\ie}~$A_n \rightarrow A_{n+1}$.
...
\\
    ...
    \\
    \textbf{(S3):}  & We define $\beta$ and $\gamma$ as the rates of stochastic conversion of \khi to \klo and GFP$^+$ to GFP$^-$. \\ %, respectively.
...
%as tracking local densities in dynamical PDE systems is particularly challenging.
...
%In addition, since the mosaic model tracks groups of cells, it has significantly smaller computational load than typical stochastic simulation algorithms, such as agent-based models or Doob–Gillespie solvers, which track individual cells or reactions. % and therefore have heavy computational load. % instead of cohorts of cells binned as age-classes. 
...
Models and the sampler will be coded in \textit{C++} programming language. % for efficient simulations and sampling.
...
define ODE systems to predict their mean behavior. %following the law of large numbers.  
...
Therefore, the density of cells with division number $q$ can be defined as $w_{q}(t, g) = u_{q}(t, 2^{-n} g)$, where $u_{q} \ge 0$. % for $0 \le g \le 1$.
